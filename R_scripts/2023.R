...
twentythree <- read.delim("2023_all.txt", header = TRUE)
twentythree$Site <- str_remove(twentythree$Site, "^S\\.")

unique(twentythree$Site)
sort(unique(twentythree$Date))
...
tthree_site <- twentythree %>%
  dplyr::filter(Site %in% c('121', '213', '232', '233', '321', '332')) %>% # 6 2023 sites
  dplyr::filter(
    (Time >= "05:30:00" & Time <= "09:00:00") | 
      (Time >= "17:00:00" & Time <= "20:30:00")) %>% # dawn and dusk times 
  mutate(habitat = case_when(
    Site %in% c("233", "213", "321") ~ "edge",
    Site %in% c("232", "121", "332") ~ "interior",
    TRUE ~ NA_character_))

unique(tthree_site$Site)    
table(tthree$Site)
sort(unique(tthree_site$Date))
...
sort(table(tthree_filter$Common.Name), decreasing = TRUE)

# ============================================================================
# 3. CREATE BIRDNET VALIDATION DATASET
# ============================================================================

# filter out species with <20 detections
tthree_under20 <- tthree_filter %>%
  group_by(Common.Name) %>%
  summarise(detections = n()) %>%
  filter(detections < 20) %>%
  ungroup()

#write.csv(tthree_under20, "2023_species_under20_counts.csv", row.names = FALSE)

### Stratified sampling based on confidence scores for species validation
set.seed(123) 

# First, calculate total detections per species
species_totals <- tthree_filter %>%
  group_by(Common.Name) %>%
  ...
  species_stratified <- species_data %>%
    mutate(
      Stratum = case_when(
        ...
        TRUE ~ "High"  # Handle edge case where Confidence exactly equals upper quantile
      )
    )
...
verification_sample <- tthree_filter %>%
  group_by(Common.Name) %>%
...
# Export to CSV
output_filename <- paste0("all2023_strata_samples_", Sys.Date(), ".csv")
...
bird_data <- read.csv("2023_strata_done.csv")
...
filtered_tthree <- filter_new_data_by_thresholds(tthree_filter, thresholds, confidence_col = "Confidence")

# 5. Check results
cat("\nUnique species in filtered dataset:\n")
print(sort(unique(filtered_tthree$Common.Name)))

# 6. Save the filtered dataset
write.csv(filtered_tthree, "2023_filtered.csv", row.names = FALSE)
...
# ============================================================================
# 1. POST-VALIDATION REMOVAL AND SWAPS
# ============================================================================

f_2023 <- read.csv("2023_filtered.csv")
unique(f_2023$Common.Name)
sort(unique(f_2023$Date))

# Species removed because of unlikely detection/swapped subspecies

# Remove specified species and rename Green-winged teal to Eurasian teal
f_2023_species <- f_2023 %>%
...
precalls_2023 <- f_2023_species
write.csv(precalls_2023, 'precalls_2023.csv')

detections_2023 <- precalls_2023 %>%
  group_by(Common.Name) %>%
  summarise(detections = n()) %>%
  ungroup()
write.csv(detections_2023, 'detections_2023.csv')

unique(f_2023_species$Common.Name)
...
callspermin_2023 <- f_2023_species %>%
...
# add habitat
calls_habitat_2023 <- callspermin_2023 %>%
  mutate(habitat = case_when(
    Site %in% c("233", "213", "321") ~ "edge",
    Site %in% c("232", "121", "332") ~ "interior",
    TRUE ~ NA_character_))

write.csv(calls_habitat_2023, '2023_statsdf.csv', row.names = FALSE)

df_2023 <- read.csv('2023_statsdf.csv', header = TRUE)

# Finding the species removed i.e. the species present in tthree_filter but not in df_2023

missing_2023 <- tthree_filter %>%
  anti_join(df_2023, by = "Common.Name") %>%
  distinct(Common.Name)

write.csv(missing_2023, 'missing_2023.csv')
