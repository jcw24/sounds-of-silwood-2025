...
# ANOVA to test habitat effect
anova(model_pcoa1, test = "F")
anova(model_pcoa2, test = "F")

# 1. Species Richness and Sampling Effort per Site
species_richness <- lump %>%
  group_by(Site, habitat) %>%
  summarise(
    species_richness = n_distinct(Common.Name),
    total_detections = n(),
    minutes_recorded = n_distinct(Source_File),
    .groups = 'drop'
  )

# Check sampling effort across sites
print("=== SAMPLING EFFORT BY SITE ===")
sampling_effort <- lump %>%
  group_by(Site) %>%
  summarise(
    minutes_recorded = n_distinct(Source_File),
    total_detections = n(),
    detections_per_minute = round(n() / n_distinct(Source_File), 2)
  ) %>%
  arrange(desc(minutes_recorded))

print(sampling_effort)
